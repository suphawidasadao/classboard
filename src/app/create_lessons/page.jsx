...
const defaultCoverSvg = "/default_cover.svg"; // path ไปยังไฟล์ SVG ของคุณ

const createDefaultLesson = (lessonNumber) => ({
  title: ``,
  coverImageFile: null,  
  coverPreviewUrl: defaultCoverSvg, 
...
export default function CreateLessonsPage() {
  ...
  // 🔹 สร้าง File จาก default cover ถ้า user ไม่เลือก
  const appendCoverFile = async (formData, lesson, lIdx) => {
    let file = lesson.coverImageFile;

    if (!file) {
      const response = await fetch(defaultCoverSvg);
      const blob = await response.blob();
      file = new File([blob], "default_cover.svg", { type: "image/svg+xml" });
    }

    formData.append(`lessons[${lIdx}][coverImage]`, file);
  };
  ...
  const saveToBackend = async () => {
    ...
    for (let lIdx = 0; lIdx < lessons.length; lIdx++) {
      await appendCoverFile(formData, lessons[lIdx], lIdx);
    ...
    try {
      ...
      if (res.ok) {
        alert('เผยแพร่บทเรียนเรียบร้อยแล้ว');
        router.push('/lesson_myPublished');
      } else {
            ...
            <label className="cursor-pointer relative">
              {lesson.coverPreviewUrl ? (
                <div className="relative">
                  <img
                    src={lesson.coverPreviewUrl || defaultCoverSvg}
